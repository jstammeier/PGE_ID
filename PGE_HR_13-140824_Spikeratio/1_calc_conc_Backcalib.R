...
reversecalib_start_str <- "2024-06-18"
reversecalib_end_str <- "2024-08-17"
...
for (i in elements_ID_df$element) {
  ...
  # i.e. the "normal" reference standard that is here used as assay standard, i.e. for calibration: LK_1ppb_QC_number
  results_int_ms$avg_natural_abd_ref = colMeans(results_int_ms[grep("^Calib_1", results_int_ms$id), "abd_ref"])
  results_int_ms$avg_natural_abd_spike = colMeans(results_int_ms[grep("^Calib_1", results_int_ms$id), "abd_spike"])
...
target_ids <- c("C005+Sp", "C01+Sp", "C05+Sp", "C1+Sp", "CO1+Sp2", "CO5+Sp") 
