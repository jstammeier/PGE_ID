...
reversecalib_start_str <- "2025-02-17"
reversecalib_end_str <- "2025-02-19"
...
results_int_ms$time <- as.Date(anytime(results_int_ms$time), format = "%Y-%m-%d")
# results_int_ms <- filter(results_int_ms, time == reversecalib_date_date)
...
cols_to_convert <- names(results_int_ms)[2:ncol(results_int_ms)]
...
for (i in elements_ID_df$element) {
  ...
  # results_int_ms$avg_natural_abd_ref = colMeans(results_int_ms[grep("^Calib$", results_int_ms$id), "abd_ref"])
  # results_int_ms$avg_natural_abd_spike = colMeans(results_int_ms[grep("^Calib$", results_int_ms$id), "abd_spike"])
  results_int_ms$avg_natural_abd_ref <- 0.6277#colMeans(as.matrix(matching_sample[, "abd_ref", drop = FALSE]), na.rm = TRUE) 
  results_int_ms$avg_natural_abd_spike <- 0.3723#colMeans(as.matrix(matching_sample[, "abd_spike", drop = FALSE]), na.rm = TRUE)
