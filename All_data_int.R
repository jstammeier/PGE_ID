...
getwd()
folders <- list.dirs(
  path = paste0(getwd(), "\\To_be_Evaluated"), recursive = T) 
...
read_file <- function(fn) {
  ...
  temp <- readLines(fn)
  temp2 <- head(temp[-(2:3)], - 8) #deletes unnecessary line in ASC files
  ...
  # delete large errors
  # rawdata[, "Intensity.AVG"] <- k(rawdata[, "Intensity.RSD"]
...
# results_conc_ms <- results_int_ms
#######################
end <- Sys.time()
